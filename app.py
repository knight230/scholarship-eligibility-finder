...
def eligibility():
    if not session.get("student_id"):
        flash("Please sign in to check your eligibility.")
    ...
    # If a student is logged in, save these details to their profile
    # so their dashboard reflects the same matches next time they visit.
    student_id = session.get("student_id")
    if student_id:
        students.update_one({"_id": ObjectId(student_id)}, {"$set": criteria})
...
@app.route("/apply/<scholarship_id>")
def apply(scholarship_id):
    if not session.get("student_id"):
        flash("Please sign in before applying.")
        return redirect(url_for("login"))

    scholarship = scholarships.find_one({"_id": ObjectId(scholarship_id)})
    if not scholarship:
        return redirect(url_for("results"))

    student = students.find_one({"_id": ObjectId(session["student_id"])})

    # Record the application (or update the timestamp if they click again)
    applications.update_one(
        {"student_id": session["student_id"], "scholarship_id": scholarship_id},
        {"$set": {
    ...
    return redirect(scholarship.get("apply_url") or url_for("results"))
...
def signup():
    ...
    if not name or not email or len(password) < 6:
        flash("Please fill in your name, email, and a password of at least 6 characters.")
        return redirect(url_for("signup"))

    if students.find_one({"email": email}) or admins.find_one({"email": email}):
    ...
    session["student_id"] = str(result.inserted_id)
...
def admin_students():
    ...
    all_students = list(students.find().sort("_id", -1))
    return render_template("admin-students.html", students=all_students, student_count=len(all_students))
...
def admin_dashboard():
    ...
    recent_applications = list(applications.find().sort("applied_at", -1).limit(20))
